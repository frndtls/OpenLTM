...
class TimeAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, n_vars, n_tokens, tau=None, delta=None, patch_mark=None):
        B, L, H, E = queries.shape
        _, S, _, D = values.shape
...
class TimeRoPEAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, n_vars, n_tokens, tau=None, delta=None, patch_mark=None):
        ...
        queries, keys = self.qk_proj(
            queries, keys, query_id=seq_id, kv_id=seq_id, patch_mark=patch_mark)

        scale = self.scale or 1. / sqrt(E)

        var_id = torch.arange(n_tokens)
        var_id = repeat(var_id, 'L -> b h L', b=B, h=1).to(queries.device)
            ...
            if attn_mask is None:
                if self.covariate:
                    attn_mask = TriangularCausalMask(
                        B, n_tokens, device=queries.device)
                else:
                    attn_mask = TriangularCausalMask(
                        B, n_tokens, device=queries.device)
            attn_mask = attn_bias.masked_fill(attn_mask.mask, float("-inf"))
